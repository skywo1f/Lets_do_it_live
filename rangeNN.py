...
logo = pygame.image.load('ops_logo_300x65.jpg')
...
send_serial_cmd("\nSet 8 outputs ", Ops241B_Eight_Ranges)
...
def read_and_render():
 ...
 #                   print(maybe_nums)
                    ...
                    range_available = False
        #give data to neural network
                print(maybe_nums)
                aList = np.asarray(maybe_nums)
                fSomething = aList[:].astype(np.float32)
                arrayNums = np.asarray(fSomething)
                try:
                        scaledArray = scaler.transform(arrayNums.reshape(1,-1))
                        tensorNums = torch.from_numpy(scaledArray)
                        tensorNums = tensorNums.type(torch.float)
                        print(len(tensorNums))
        #                print(tensorNums.size)
        #                print(tensorNums)       
                        outputs = model(tensorNums)
                        print(outputs)
                        if outputs < 0.5:
                                print("no ring")
